        ...
        if form.is_valid():
            try:
                ...
                try:
                    ...
                    return render(request, 'encrypt/encrypt_done.html', {'result': y})
                except:
                    messages.success(request, 'Something went wrong try after some time.')
                    return redirect('encrypt:encrypt')
            except:
                messages.warning(request, 'Something went wrong try after some time.')
                return redirect('encrypt:encrypt')
    return render(request, 'encrypt/encrypt.html', {'form': form})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, "Your text is decrypted")
                return render(request, 'encrypt/decrypt_done.html', {'result': data})
            except:
                messages.warning(request, 'Incorrect password or text.')
                return redirect('encrypt:decrypt')
        else:
            messages.success(request, "Incorrect password or text")
            return redirect('encrypt:decrypt')
    return render(request, 'encrypt/decrypt.html', {'form': form})


def encrypted(request):
    return render(request, 'encrypt/encrypt_done.html')


def decrypted(request):
    return render(request, 'encrypt/decrypt_done.html')